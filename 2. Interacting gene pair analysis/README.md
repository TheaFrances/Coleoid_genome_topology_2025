...
- [Plot genomic distance vs. genomic distance to define the interaction frequency threshold](#Plot-genomic-distance-vs.-genomic-distance-to-define-the-interaction-frequency-threshold)

This folder documents the interacting gene pair analyses. It includes gene-bin mapping, extracting orthologous gene pairs between species, classifying their chromosomal distribution in *P. maximus*. It also details the extraction of interaction frequencies and genomic distances between gene pairs across species, which we based the interaction frequency threshold on. All steps are demonstrated using the *E. scolopes* sample 403493 at 100 kb resolution, which is later merged with the *O. bimaculoides* interaction matrix at 50 kb resolution and the *S. officinalis* interaction matrix at 100 kb resolution. For the species *S. officinalis*, no gene annotation was available for the *S. officinalis* reference genome at the time of writing this paper. Therefore, additional commands are provided at the start to classify orthologous genes and for some downstream analyses.
...
```
## Plot genomic distance vs. genomic distance to define the interaction frequency threshold for interacting gene pairs

The interaction frequency threshold for interacting gene pairs weas determined by examining scatterplots generated by the R scripts [`get_interaction_threshold_logged_eupsc_sepof.R`](get_interaction_threshold_logged_eupsc_sepof.R). This example is comparing *E. scolopes* and *S. officinalis*, but a similar method was used for comparing *E. scolopes* and *O. bimaculoides*.
